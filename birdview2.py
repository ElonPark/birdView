...
def matching_couples(value, count):
    ...
    for i, hobbys1 in np.ndenumerate(people):
        for j, hobbys2 in np.ndenumerate(people):
            if i[0] == j[0]:
                continue

            is_include_couple = "{}-{}".format(j[0], i[0])
            if is_include_couple in coupleDic:
                continue
            ...
            if count > 0:
                coupleDic["{}-{}".format(i[0], j[0])] = True
                coupleTuple = (i[0], j[0], intersection)
...
def main():
    ...
    if mode == 1:
        value = np.array(self_input_mode())
